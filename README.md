...
### 3. ONNX 埋め込み層における境界外アクセス（Gather）クラッシュの調査と対策
- **現象**: テキスト Embedding 語彙サイズを `50257` に修正したことに伴い、Turboモデルの日本語テキスト合成（「。」などのテキストトークンID `16764` が発生）において、`embed_tokens.onnx` 内部の `/speech_emb/Gather`（語彙サイズ 6563 の音声埋め込みテーブル）で境界外アクセス例外（`idx=16764 must be within [-6563, 6562]`）が発生する問題を確認しました。
- **原因と対策**: ONNX Runtime は、条件付き（Where）等で結果が棄却されるブランチであっても全ノードを先行評価（Eager evaluation）するため、テキストトークンが `/speech_emb/Gather` に流れ込み境界外アクセスを引き起こします。現在、`uv` を用いた環境上で ONNX モデルの修復および C# 側の対策を進めています。
