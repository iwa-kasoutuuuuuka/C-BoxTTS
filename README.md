...
| **テキスト処理・正規化** | **日本語専用セーフガード**<br>・Unicode NFC 正規化による濁音・半濁音分離バグ修正<br>・インデックス超過 (2453超) 未知文字の自動クリップ回路 | **English Normalizer**<br>・略語展開 (`Mr.`, `Dr.`, 月名略語等)<br>・序数 (`1st`→`first`)・時刻 (`3:00`→`three o'clock`)<br>・年号 (`2024`→`twenty twenty-four`)・分数 (`1/2`→`one half`)<br>・頭字語 (`FBI`, `NASA` 等) を不自然に分解せず滑らかに発音保持 / URL・email除去<br>・通貨 (`$1.50`→`one dollar and fifty cents`, `£100`→`one hundred pounds`) |
