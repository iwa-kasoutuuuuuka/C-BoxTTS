...
### 7. 話速制御（タイムストレッチ）における速度ドリフト・逆転の修正
- **不具合内容**: 音声伸縮（タイムストレッチ）アルゴリズム（SOLA）において、ピッチ周期を同期させるための微調整ズレ（`bestDelay`）が次のフレームの基準位置に累積されていました。これにより長文合成時にズレが何百回も累積し、**「100%（等倍）より90%（ゆっくり）のほうが早く再生が完了してしまう」** といった速度の逆転現象や著しい話速の不安定化が発生していました。
- **対策**: 目標出力配置位置を示すポインタを `bestDelay` の影響を受けないよう静的に（一定のステップ間隔で）進める管理へ変更しました。これにより、ピッチの位相を綺麗に合わせつつ、再生時間は設定した話速（`speed`）に正確に一致・安定するようになりました。
