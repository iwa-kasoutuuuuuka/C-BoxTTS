...
トークナイザーが `tokenizer_mtl.json` から文字IDを引き当てる際、ONNXモデルの埋め込み層上限インデックス（**`2453`**）を超える未知文字や全角記号（例：全角の「？」）が検出された場合、クラッシュを防止するために自動的に未知語トークン `1 (UNK)` にクリップ・マッピングする安全ガード回路を搭載しています。

### ③ 言語識別トークンIDバグの解消
Multilingualモデルにおける英語の言語識別トークンID指定を正しい値（**`708`**）へ修正し、多言語モデルで英語読み上げ時に発生していた配列範囲外例外（`indices element out of data bounds`）を完全に解消しました。
...
### ⑤ 英文の音声合成精度向上とテキスト正規化 (English Normalizer)
英文の読み上げ時において、大文字・小文字の不一致によりトークンマッチング（BPE）が機能しなくなる問題を防ぐため、事前エンコード処理における小文字化（`ToLowerInvariant`）を徹底。また、Python版に準拠した記号のクリーンアップや文末ピリオド自動補完（`punc_norm`）を移植しました。さらに、数値（例: `123` -> `one hundred twenty-three`）、略語（`Mr.` -> `mister`, `Dr.` -> `doctor` 等）、一部記号（`%` -> `percent`, `$` -> `dollars`）を自動的にフルスペル展開する `EnglishNormalizer` を独自搭載し、英語音声の品質を向上させました。
