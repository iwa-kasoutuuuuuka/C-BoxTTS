...
namespace CBoxTTS.Native
{
    public static class EnglishNormalizer
    {
        ...
        private static readonly HashSet<string> KnownCommonWords = new(StringComparer.Ordinal)
        {
            ...
            "SHOW", "SIDE", "TURN", "USED",
            "AGO", "AGE", "AIR", "ARM", "ART", "ASK", "BIG", "BIT", "BOX",
        ...
        private static Dictionary<string, string> UserDictionary = new(StringComparer.OrdinalIgnoreCase);
        private static List<(Regex Regex, string Value)> UserDictionaryCompiled = new();
        ...
        public static string Normalize(string text)
        {
            ...
            // ユーザー辞書による置換 (スナップショットを安全にイテレーション)
            var userDictList = UserDictionaryCompiled;
            foreach (var item in userDictList)
            {
            ...
            {
                text = item.Regex.Replace(text, item.Replacement);
            }

            // 2. 序数の変換
            text = OrdinalRegex.Replace(text, m =>
            {
                if (int.TryParse(m.Groups[1].Value, out int num) && num > 0 && num <= 999999)
                {
        ...
        private static string NumberToOrdinalWord(int num)
        {
            ...
            if (lastTwoDigits == 0)
            {
                string baseWords = NumberToWords(num);
                if (baseWords.EndsWith(" hundred"))
                {
                    return baseWords.Substring(0, baseWords.Length - 8) + " hundredth";
                }
                if (baseWords.EndsWith(" thousand"))
                {
                    return baseWords.Substring(0, baseWords.Length - 9) + " thousandth";
                }
                if (baseWords.EndsWith(" million"))
                {
                    return baseWords.Substring(0, baseWords.Length - 8) + " millionth";
                }
                if (baseWords.EndsWith(" billion"))
                {
                    return baseWords.Substring(0, baseWords.Length - 8) + " billionth";
                }
                if (baseWords.EndsWith(" trillion"))
                {
                    return baseWords.Substring(0, baseWords.Length - 9) + " trillionth";
                }
                return baseWords + "th";
            }
            else
            {
                int baseNum = (num / 100) * 100;
                string baseWords = NumberToWords(baseNum);
                string suffix = NumberToOrdinalWord(lastTwoDigits);
                return $"{baseWords} and {suffix}";
            }
        }
        ...
        private static string NumberToWords(long number)
        {
            ...
            if (number < 0)
            {
                ...
                {
                    return "minus " + NumberToWordsUlong(9223372036854775808UL);
                }
                return "minus " + NumberToWordsUlong((ulong)Math.Abs(number));
            }
        ...
        private static string NumberToWordsUlong(ulong number)
        {
            ...
            if ((number / 1000000000000000000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000000000000000000UL) + " quintillion ");
                number %= 1000000000000000000UL;
            }
            if ((number / 1000000000000000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000000000000000UL) + " quadrillion ");
                number %= 1000000000000000UL;
            }
            if ((number / 1000000000000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000000000000UL) + " trillion ");
                number %= 1000000000000UL;
            }
            if ((number / 1000000000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000000000UL) + " billion ");
                number %= 1000000000UL;
            }
            if ((number / 1000000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000000UL) + " million ");
                number %= 1000000UL;
            }
            if ((number / 1000UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 1000UL) + " thousand ");
                number %= 1000UL;
            }
            if ((number / 100UL) > 0)
            {
                words.Append(NumberToWordsUlong(number / 100UL) + " hundred ");
                number %= 100UL;
            }
        ...
        public static void LoadUserDictionary(string baseDir)
        {
            lock (_lock)
            {
                ...
                var newCompiled = new List<(Regex Regex, string Value)>();
                ...
                if (System.IO.File.Exists(path))
                {
                        ...
                        foreach (var line in lines)
                        {
                            ...
                            if (parts.Length >= 2)
                            {
                                ...
                                if (!string.IsNullOrEmpty(word))
                                {
                                    ...
                                    var regex = new Regex($@"\b{escapedWord}\b", RegexOptions.IgnoreCase | RegexOptions.Compiled);
                                    newCompiled.Add((regex, read));
                                }
                            }
                        }
                    }
                    ...
                    {
                        // ログ記録等の処置が可能
                        System.Diagnostics.Debug.WriteLine($"Failed to read English user dictionary: {ioEx.Message}");
                    }
                }
                
                UserDictionary = newDict;
                UserDictionaryCompiled = newCompiled;
